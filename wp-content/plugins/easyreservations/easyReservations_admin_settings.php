...
function reservation_settings_page(){
	...
	if($setting_current_page == "form"){//Get current form Options
		...
		$formresult = $wpdb->get_results($wpdb->prepare("SELECT option_name, option_value FROM ".$wpdb->prefix ."options WHERE option_name like '%1\$s' ", $wpdb->esc_like("reservations_form_") . '%'));
