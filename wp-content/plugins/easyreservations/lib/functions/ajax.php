...
function easyreservations_send_table_callback() {
	...
	if(!empty($search)){
		...
		foreach($searches as $search_res){
			...
			if(preg_match('/^[0-9]+$/i', $search_res))
				...
				$search_str .= "(name like '%1\$s' OR email like '%1\$s' OR custom like '%1\$s'$roomsearch)";
			}
			$st++;
	...
	if(empty($search_str)) $search_str = ' %n ';
	$items1 = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM ".$wpdb->prefix ."reservations WHERE approve='yes' $zeichen $selectors $search_str $permission_selectors", $esc_like));
	$items2 = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM ".$wpdb->prefix ."reservations WHERE approve='no' $zeichen $selectors $search_str $permission_selectors", $esc_like));
	$items3 = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM ".$wpdb->prefix ."reservations WHERE approve='' $zeichen $selectors $search_str $permission_selectors", $esc_like));
	$items4 = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM ".$wpdb->prefix ."reservations WHERE approve='yes' AND departure < NOW() $selectors $search_str $permission_selectors", $esc_like));
	$items5 = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM ".$wpdb->prefix ."reservations WHERE approve='del' $selectors $search_str $permission_selectors", $esc_like));
	$items7 = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM ".$wpdb->prefix ."reservations WHERE approve='yes' AND NOW() BETWEEN arrival AND departure $selectors $search_str $permission_selectors", $esc_like));
	$items6 = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM ".$wpdb->prefix ."reservations WHERE 1=1 $selectors $search_str $permission_selectors", $esc_like));
	if(isset($favourite_sql)) $countfav = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM ".$wpdb->prefix ."reservations WHERE $favourite_sql $selectors $search_str $permission_selectors", $esc_like));
	...
	if(isset($month_selector) || isset($room_selector) || isset($status_selector)){
		$variableitems = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM ".$wpdb->prefix ."reservations WHERE $type $selectors $zeichen $search_str $permission_selectors", $esc_like));
		$items=$variableitems;
	}
