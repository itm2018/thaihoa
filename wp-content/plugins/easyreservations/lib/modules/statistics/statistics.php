						...
						if(!$only || $only < 2){
							...
							foreach($the_rooms_array as $resource){
								$table .= '<span>';
									...
									$table .= '<b><span>'.$thisarray['resource'][$resource->ID].'</span> '.WPGlobus_Core::extract_text(__($resource->post_title)).'</b>';
	...
	<?php
		...
		if($count_all_reservations > 0){
			...
			foreach($the_rooms_array as $room){
				...
				$resourceJSarray.='{ label: "'.WPGlobus_Core::extract_text(__($room->post_title)).' - '.$count.'",  data: [[1,'.$amount.']]},';
			}
