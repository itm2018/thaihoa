...
function easyreservations_send_multipart_mail($content){
	add_filter('wp_mail_content_type',create_function('', 'return "multipart/alternative";'));
	$mail_contents = explode('<--HTML-->', $content);
	$message = array('text/plain' => '', 'text/html' => '');
	$message['text/plain'] = html_entity_decode(str_replace('<br>', '\n', str_replace(array(']', ']'), '', $mail_contents[0])));
	if(isset($mail_contents[1]) && !empty($mail_contents[1])) $message['text/html']  .= $mail_contents[1];
	else $message['text/html'] .= $mail_contents[0];
