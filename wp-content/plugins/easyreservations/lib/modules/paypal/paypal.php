...
if(is_admin()){
	...
	function easyreservations_pay_add_settings(){
		if(isset($_GET['site']) && $_GET['site'] == "pay"){
			...
			$deposit_options = '<input type="checkbox" name="deposit_activate" style="margin-bottom:2px" '.checked(1, $options['deposit']['on'],false).'> '.__('Activate', 'easyReservations').'<br><input type="checkbox" name="deposit_payfull" '.checked(1, $options['deposit']['full'],false).'> '.__('Guest can choose to pay full price','easyReservations').'<br><input type="checkbox" name="deposit_payown" '.checked(1, $options['deposit']['own'],false).'> '.__('Guest can define the discount', 'easyReservations').'<br><input type="checkbox" name="deposit_payperc"  '.checked(1, $options['deposit']['perc'],false).'> '.__('Guest can choose to pay:', 'easyReservations').' (<a onclick="easy_add_deposit()" style="cursor: pointer">'.__('Add', 'easyReservations').'</a>)<br><span id="discount_selection" style="margin-left:10px;display:inline-block;">'.$deposit_options_value.'</span>';
			...
			$rows = array(
				...
				__( 'After success return to', 'easyReservations' ) => 'Text: <input type="text" style="width:150px" name="er_authorize_return_text" value="'.(!empty($aoptions['return'])? $aoptions['return'] : null).'"> URL:<select id="er_authorize_return" name="er_authorize_return" onchange="erReturn(this,\'authorize\');"><option value="useredit" '.selected($auth_return, 'useredit', false).'>'.__( 'User Control Panel' , 'easyReservations' ).'</option><option value="page" '.selected(is_numeric($auth_return), true, false).'>'.__( 'Page' , 'easyReservations' ).'</option><option value="own" '.selected((!is_numeric($auth_return) && $auth_return != 'useredit') ? true : false, true, false).'>'.__( 'URL' , 'easyReservations' ).'</option></select><span id="er_authorize_return_2nd"></span>',
				__( 'Cancel', 'easyReservations' ) => 'Text: <input type="text" style="width:150px" name="er_authorize_cancel" value="'.(!empty($aoptions['cancel']) ? $aoptions['cancel'] : null).'"> URL: <input type="text" style="width:300px" name="er_authorize_cancel_url" value="'.(!empty($aoptions['cancel_url'])? $aoptions['cancel_url'] : null).'">',
				__( 'Button', 'easyReservations' ) => easyreservations_generate_input_select('er_authorize_button',array('own' => __( 'Select' , 'easyReservations' ),'https://content.authorize.net/images/buy-now-gold.gif' => 'Simple Checkout Gold','https://content.authorize.net/images/buy-now-blue.gif' => 'Simple Checkout Blue'),(!empty($aoptions['button']) ? $aoptions['button'] : null),'onchange="changeImg(this,\'authorize\');"').'<input onchange="changeImg(this,\'authorize\');" type="text" id="er_authorize_button_url" name="er_authorize_button_url" style="width:400px"  value="'.(!empty($aoptions['button']) ? $aoptions['button'] : null).'"><span id="er-authorize-button-img" style="float:right;vertical-align:text-bottom;width:200px;text-align:right;"><img src="'.(!empty($aoptions['button']) ? $aoptions['button'] : null).'"></span>',
			);
			...
			$rows = array(
				...
				__( 'Skip review page', 'easyReservations' ) => '<input type="checkbox" name="er_2checkout_skip" value="1" '.checked($coptions['skip'],1,false).'> '.__('Skip the order review page of the purchase routine', 'easyReservations'),
			...
			$rows = array(
				'col' => __('To automatically set the paid amount and approve you have to log in your google account, go to', 'easyReservations').' <strong>Settings -> Integration</strong>, '.__('enter' , 'easyReservations').' <code>'.RESERVATIONS_URL.'lib/modules/paypal/google_ipn.php</code> '.__('as API callback URL and select <strong>Notification Serial Number</strong> thereafter.', 'easyReservations'),
				...
				__( 'Name' , 'easyReservations' ) => '<input type="text" style="width:200px" name="er_wallet_name" value="'.$goptions['name'].'"> '.__('Available tag: [resource]', 'easyReservations'),
